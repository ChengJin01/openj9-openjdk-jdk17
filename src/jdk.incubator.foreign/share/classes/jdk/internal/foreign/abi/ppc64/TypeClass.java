...
public enum TypeClass {
	...
	private static ValueLayout longLayout = (osName.contains("aix") || osName.contains("win")) ? C_LONG_LONG : C_LONG;
	...
	public static VarHandle classifyVarHandle(ValueLayout layout) {
		 ...
		 */
		if ((carrier == byte.class) || (carrier == short.class)
		|| (carrier == int.class) || (carrier == long.class)
		) {
			argHandle = longLayout.varHandle(long.class);
		} else if (carrier == float.class) {
			argHandle = C_DOUBLE.varHandle(double.class);
		} else if ((carrier == double.class) || (carrier == MemoryAddress.class)) {
	...
	private static Class<?> classifyValueLayoutCarrier(ValueLayout layout) {
		...
		case POINTER:
			...
			break;
		default:
			throw new IllegalArgumentException("The layout's ABI Class is undefined: layout = " + layout); //$NON-NLS-1$
		}
